...
% true parameters
theta0_init_true = [theta_0(1) theta_0(2)];
%theta0_init_sampled = [Q0_init R0_init]; 
...
theta_samples = zeros(T, 2);
phi_samples = zeros(T, 2);
...
disp('Initial theta:');
disp([theta_samples(1, 1).^2, theta_samples(1, 2).^2]);

%cov_matrix = diag(alpha * ones(1, 6));

% Choice of standard deviations are hard-coded
std_matrix = diag(sqrt([alpha/2 alpha/10])); % Linear model
%std_matrix = diag(sqrt([alpha/10 alpha/10]));

% Using parameters obtained from a pilot simulation run as per the used
% formula: 2.565^2/3 * empirical_var(theta_pilotRun) 
%std_matrix = diag(sqrt([0.0259    0.0019]));
...
%m_theta_proposed_init.c = c0_init; % Keep this parameter fixed
%m_theta_proposed_init.d = theta_samples(1, 4); 
m_theta_proposed_init.Q = theta_samples(1, 1).^2; 
m_theta_proposed_init.R = theta_samples(1, 2).^2;
...
for i=2:T
    ...
    phi_proposed = phi_samples(i-1, :) + randn(1, 2) * A;
    theta_proposed = h_vector(phi_proposed);
    
    %m_theta_proposed.a = theta_proposed(1); m_theta_proposed.b = theta_proposed(2); 
    %m_theta_proposed.c = c0_init; % Keep this parameter fixed
    %m_theta_proposed.d = theta_proposed(4); 
    m_theta_proposed.Q = theta_proposed(1).^2; m_theta_proposed.R = theta_proposed(2).^2;
    ...
    if log_acceptance_rate > log(rand(1,1))
        ...
        theta_samples(i, :) = [sqrt(m_theta_proposed.Q) sqrt(m_theta_proposed.R)];
...
%     if mod(i-1, 1000) == 0
    if mod(i-1, 2000) == 0
        disp(['Iteration nr: ' num2str(i-1) ' Estimates, Q: ',num2str(theta_samples(i,1).^2),'  R: ',num2str(theta_samples(i,2).^2)])
...
function [phi_vector] = g_vector(theta_vector)
    ...
    phi_vector(1) = g(theta_vector(1), sqrt(0.1), sqrt(1.5)); % [0.1, 1.5] Variance Q
    phi_vector(2) = g(theta_vector(2), sqrt(eps), sqrt(1.0)); % [eps, 1.0] Variance R
    
end

function [theta_vector] = h_vector(phi_vector)
    theta_vector = zeros(1, length(phi_vector));
    theta_vector(1) = h(phi_vector(1), sqrt(0.1), sqrt(1.5)); % [0.1, 1.5] Variance Q
    theta_vector(2) = h(phi_vector(2), sqrt(eps), sqrt(1.0)); % [eps, 1.0] Variance R
