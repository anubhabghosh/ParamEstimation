...
function [g, approxLogLik] = pf_new(m,M,z)
  ...
  for t=1:N
    ...
    %x = m.a*(x./(1+x.^2)) + cos(1.2*t) + sqrt(m.Q)*randn(1,M); % 4. Predict the particles (state equation)
    x = m.a*(x./(1.0+ (0.2.*x).^2)) + u(t) + sqrt(m.Q)*randn(1,M); % 4. Predict the particles (state equation)
