...
% (using expectation maximization). Some sections have been adopted from
% the code necessary for running the nonlinear example 
...
%
% Mainly functions as a kind of wrapper
...
function predicted_theta_vector = evaluate_using_NLSS_fixed_theta_simpler_model_random_init(y, theta_actual, N)

%======================================================================
% Initialize the starting vector randomly within the uniform prior
%======================================================================
a0 = 0.0 + (1.0 - 0.0) * rand(1);
b0 = 0.1 + (2.0 - 0.1) *rand(1);
Q0 = eps + (1.0 - eps) *rand(1);
R0 = eps + (1.0 - eps) *rand(1);

% a0 = random_init(theta_actual(1), 0.5, 0.0, 1.0);
% b0 = random_init(theta_actual(2), 0.5, 0.1, 2.0);
% Q0 = random_init_var(theta_actual(3), 0.5, eps, 1.0);
% R0 = random_init_var(theta_actual(4), 0.5, eps, 1.0);
...
disp('----------------------------------------------------------------------------------------------------------');
disp(['True parameter a: ' num2str(theta_actual(1)),'  b: ',num2str(theta_actual(2)), ...
    '  Q: ',num2str(theta_actual(3)),'  R: ',num2str(theta_actual(4))])
